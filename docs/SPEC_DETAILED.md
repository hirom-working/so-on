...
### 2.3 フォント
- **UIテキスト**: モダンミニマル（シンプルなサンセリフ体）
- **カセットラベル**: 手書き風フォント（Caveat）
- **用途**: ラベル、時間表示、ボタンテキスト
...
- **ラベル**:
  - 白いラベルエリアに手書き風フォント（Caveat）で表示
  - 形式: `{音源名}_{YYYY/MM/DD}` （例: `sentakuki_2025/12/31`）
  - 微小な傾き（-0.5deg）で手書き感を演出
...
├── src/
│   ├── components/
│   │   ├── Cassette.tsx    # カセットテープコンポーネント
│   │   ├── VUMeter.tsx     # VUメーターコンポーネント
│   │   ├── TimeDial.tsx    # 時間選択ダイヤル
│   │   └── Controls.tsx    # 再生・停止ボタン
│   ├── App.tsx             # メインアプリケーション
│   └── main.tsx            # エントリーポイント
├── public/
│   └── audio.m4a           # 音源ファイル（M4A形式）
└── index.html              # HTMLテンプレート
```

### 13.4 外部リソース
- **フォント**: Google Fonts - Caveat（手書き風）
...
| 日付 | バージョン | 内容 |
|------|-----------|------|
| 2026-01-02 | 1.1 | UI改善: 80年代風リアル質感、手書きラベル(Caveat)、Webプロトタイプ情報追加 |
